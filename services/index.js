...
const listFiles = async (host, skip, limit) => {
  ...
  // console.table(
  //   `host:${host},skip=${skip},limit=${limit},root=${root},files=${files}`
  // );
  ...
  const documents = Enumerable.from(files)
    .select(x => {
      const url = x.replace(root, "").replace(/\\/g, "/"); //const url = x.replace(root, "") -- previous value
      const parts = url.split("//").filter(x => x) || [];
      const raw = `${host}/photos${parts.join("/")}`; // removed redundant backslash
      const name = parts.pop();
      const album = name.split("/")[1]; //const album = (parts[0] || "").replace(/\b\S/g, t => t.toUpperCase());  --previos value
      const path = `/albums/${album}${name}`; // removed redundant backslash
      return {
        id: createHash("md5")
          .update(raw)
          .digest("hex"),
